...
int minDeletions(vector<int> arr) {
    ...
    if (n <= 18) {
        ...
        // Precompute LIS ending at each position and starting from each position
...
int main() {
    // Test cases
    cout << minDeletions({1, 5, 10}) << endl;      // 0
    cout << minDeletions({1, 2, 6, 4, 3}) << endl; // 1
    cout << minDeletions({3, 4, 2, 5, 1}) << endl; // 1
